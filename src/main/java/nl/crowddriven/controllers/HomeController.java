...
public class HomeController {
	...
	public String home(HttpServletRequest req, @RequestParam(defaultValue = "1", required = false) int page, ModelMap model, HttpSession session) {
		...
		if (user != null && !user.isAnonymous()) {
			...
			if (session.getAttribute("isNotLogged") != null) {

				session.removeAttribute("isNotLogged");
			}
	...
	public String getHomepageContainer(ModelMap model, HttpSession session) {
		
		return "component/home-container";
	}
	...
	@RequestMapping(value = { "divetest" })
	public String divetest(ModelMap model, HttpSession session) {

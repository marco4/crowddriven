...
public class ScoringService implements ScoringServiceIF, InitializingBean {
...

	{
		Value<GlobalStats> fetchGlobalStats = new Value<GlobalStats>() {
			...
			public GlobalStats get() {
				...
				int totalHours = tagEntryRepo.countAllHours();
				int totalScores = tagEntryRepo.countAllScores();
				List<User> mostRankUsers = userRepo.getTopRanking();
				List<TopTags> mostTagUsers = userRepo.getTopTags();
				int totalDiveSearches = tagEntryRepo.countDiveSearches();
				int totalDiveClicks = tagEntryRepo.countDiveClicks();
				int totalDiveCurated = tagEntryRepo.countDiveCurated();
				int totalUsers = tagEntryRepo.countUsers();
				int totalRegisteredUsers = tagEntryRepo.countRegisteredUsers();
				int totalRealPersonTags = tagEntryRepo.countRealPersonTags();
				int totalFictionalPersonTags = tagEntryRepo.countFictionalPersonTags();
				int totalRealLocationTags = tagEntryRepo.countRealLocationTags();
				int totalFictionalLocationTags = tagEntryRepo.countFictionalLocationTags();
				int totalEventTags = tagEntryRepo.countEventTags();
				int totalMatchedTags = tagEntryRepo.countMatchedTags();
				int totalUnmatchedTags = tagEntryRepo.countUnmatchedTags();
				int totalDictionaryTags = tagEntryRepo.countDictionaryTags();
				int totalPioneerTags = tagEntryRepo.countPioneerTags();
				int totalUniqueTags = tagEntryRepo.countUniqueTags();
				List<GameScore> lastPlayedGames = gameRepo.getRecentGamesTotal();
				List<GameScore> mostPopularGames = gameRepo.getPopularGamesTotal();
				return new GlobalStats(lastPlayedGames, countTags, countMatches, tagCloud,
						topScores, currentlyPlaying, totalHours, totalScores, mostRankUsers, 
						mostTagUsers, totalDiveSearches, totalDiveClicks, totalDiveCurated, totalUsers, 
						totalRegisteredUsers, totalRealPersonTags, totalFictionalPersonTags, totalRealLocationTags, totalFictionalLocationTags, totalEventTags, totalMatchedTags, totalUnmatchedTags, totalDictionaryTags, totalPioneerTags, totalUniqueTags, mostPopularGames);
			}
		};
	...
	/*public GlobalStats getUserStats() {
		int countTags = tagEntryRepo.countTags();
		int countMatches = tagEntryRepo.countMatches();
		List<TagCloudItem> tagCloud = tagEntryRepo.getTagCloud();
		TopScores topScores = userRepo.getTopScores();
		...
		return new GlobalStats(countTags, countMatches, tagCloud,
				topScores, currentlyPlaying, totalHours);
	}*/

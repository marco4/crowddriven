...
//	constructor: function(gameId, fragmentID, startTimeWithinEpisode, duration, startTime) {
	constructor: function(gameId, video, startTime, gameType) {
		...
		jQuery("#games-queue").attr("style", "display: none");
		jQuery('#game-image').attr("style", "position: absolute; left: 350px; z-index:2000; top:25px;");
		this.gameId = gameId;
	...
	initVideoPlayer: function()
	{
		...
		if (this.startTime > this.duration) {
			...
			jQuery("#sidebar-wrapper").attr("style", "visibility: show")
			this.endGame();
//				this.update();
		} else if (realElapsed >= 0 && this.stopGame == null) {
			...
			jQuery('#game-image').attr("style", "position: fixed; left: 25px; z-index:2000; top:25px;");
			jQuery('#inputBar').show();
			jQuery("#sidebar-wrapper").attr("style", "display: none");
			...
			mytimeout = setTimeout(jQuery.proxy(this.initVideoPlayer, this), 100);
		}
	},
...
//			this.endGame();
	...
	endGame: function() {
		...
		window.location.href = "/game/" + this.gameId + "/recap/" + this.lastKnownUserId + "/" + this.gameType;
	},

					...
					<div class="col-md-12">
						<div class="box solid">
							<div class="box-title">
								...
								<h5>Added tags per month </h5>
								<div class="tools">
								<c:choose>	
									<c:when test="${((monthTagMap['NOVEMBER']-monthTagMap['OCTOBER'])/monthTagMap['OCTOBER']*100) < 0}">
										<span class="label label-danger">
										<fmt:formatNumber type="number" value="${(monthTagMap['NOVEMBER']-monthTagMap['OCTOBER'])/monthTagMap['OCTOBER']*100}" maxFractionDigits="0" pattern="###.###" />%
					...
					<div class="col-md-12">
						<ul class="tagcloud unstyled">
							<c:forEach items="${globalStats.tagCloud}" var="tag">
								<li class="tag-entry size-${tag.relativeSize}"><a href="javascript:;" class="taglinking" data-item="/tag/${tag.normalizedTag}">${tag.normalizedTag}</a></li>
							</c:forEach>
						</ul>
					</div>
				</div>
...
<script>
	$(function() {
		...
		$('.taglinking').click(function() {
    		 ...
    		 .fail(function(msg) {
    			$('#tagadded').html('Fail to load data');
    		 });
		 });
	});
</script>

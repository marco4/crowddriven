...
public class TagEntryRepository extends AbstractRepository<TagEntry> {
	...
	public List<String> getAllTags(int videoId) {
		String q = "SELECT t.normalizedTag FROM TagEntry t "
				+ "WHERE t.game.video.id = :videoId "
				+ "GROUP BY t.normalizedTag ORDER BY COUNT(*) DESC";
		return getEntityManager().createQuery(q, String.class)
	...
	public List<String> getDiveLastSearches() {
        Query query = getEntityManager().createNativeQuery(
                "SELECT details FROM dive.AjaxLog where action = 'Search' ORDER BY action ASC LIMIT 10");
       

        return query.getResultList();
	}
	public List<String> getDiveLastClicks() {
        Query query = getEntityManager().createNativeQuery(
                "SELECT details FROM dive.AjaxLog where action = 'current entity' ORDER BY action ASC LIMIT 10");
       

        return query.getResultList();
	}
	public int countAllHours() {
		Query q = getEntityManager().createQuery("SELECT SUM(tag.game.video.duration) FROM TagEntry tag");
	...
	public int countDiveSearches() {
		String q = "SELECT COUNT(*) FROM dive.AjaxLog WHERE action LIKE 'search'";
		Object count = getEntityManager().createNativeQuery(q).getSingleResult();
		return ((Number) count).intValue();
	}
	public int countDiveClicks() {
		String q = "SELECT COUNT(*) FROM dive.AjaxLog WHERE action LIKE 'current entity'";
		Object count = getEntityManager().createNativeQuery(q).getSingleResult();
		return ((Number) count).intValue();
	}
	public int countDiveCurated() {
		String q = "SELECT COUNT(*) FROM dive.AjaxLog WHERE action LIKE 'improve entity'";
		Object count = getEntityManager().createNativeQuery(q).getSingleResult();
		return ((Number) count).intValue();
	}
	...
	public int countRegisteredUsers() {
		String q = "SELECT COUNT(*) FROM User where email is not null";
		Object count = getEntityManager().createNativeQuery(q).getSingleResult();
		return ((Number) count).intValue();
	}
	public int countRealPersonTags() {
		String q = "SELECT COUNT(tag) FROM TagEntry WHERE dictionary = 'real-person'";
		Object count = getEntityManager().createNativeQuery(q).getSingleResult();
		return ((Number) count).intValue();
	}

	public int countFictionalPersonTags() {
		String q = "SELECT COUNT(tag) FROM TagEntry WHERE dictionary = 'fictional-person'";
		Object count = getEntityManager().createNativeQuery(q).getSingleResult();
		return ((Number) count).intValue();
	}

	public int countRealLocationTags() {
		String q = "SELECT COUNT(tag) FROM TagEntry WHERE dictionary = 'real-location'";
		Object count = getEntityManager().createNativeQuery(q).getSingleResult();
		return ((Number) count).intValue();
	}

	public int countFictionalLocationTags() {
		String q = "SELECT COUNT(tag) FROM TagEntry WHERE dictionary = 'fictional-location'";
		Object count = getEntityManager().createNativeQuery(q).getSingleResult();
		return ((Number) count).intValue();
	}

	public int countEventTags() {
		String q = "SELECT COUNT(tag) FROM TagEntry WHERE dictionary = 'events'";
		Object count = getEntityManager().createNativeQuery(q).getSingleResult();
		return ((Number) count).intValue();
	}

	public int countMatchedTags() {
		String q = "SELECT COUNT(tag) FROM TagEntry WHERE matchingTagEntry_id IS NOT NULL";
		Object count = getEntityManager().createNativeQuery(q).getSingleResult();
		return ((Number) count).intValue();
	}
	public int countUnmatchedTags() {
		String q = "SELECT COUNT(tag) FROM TagEntry WHERE matchingTagEntry_id IS NULL";
		Object count = getEntityManager().createNativeQuery(q).getSingleResult();
		return ((Number) count).intValue();
	}
	public int countDictionaryTags() {
		String q = "SELECT COUNT(tag) FROM TagEntry WHERE dictionary IS NOT NULL";
		Object count = getEntityManager().createNativeQuery(q).getSingleResult();
		return ((Number) count).intValue();
	}
	public int countPioneerTags() {
		String q = "SELECT COUNT(tag) FROM TagEntry WHERE pioneer = 1 and matchingTagEntry_id != NULL";
		Object count = getEntityManager().createNativeQuery(q).getSingleResult();
		return ((Number) count).intValue();
	}
	public int countUniqueTags() {
		String q = "SELECT COUNT(DISTINCT normalizedTag) FROM TagEntry";
		Object count = getEntityManager().createNativeQuery(q).getSingleResult();
		return ((Number) count).intValue();
	}
	public int countAllScores() {
		Query q = getEntityManager().createQuery("SELECT SUM(tag.score) FROM TagEntry tag");
		Object result = q.getSingleResult();
		if (result != null) {
			return (int) (long) (Long) q.getSingleResult();
		}		
	...
	public int countUnmatchedTagsByUser(int ownerId) {
		Query q = getEntityManager().createQuery("SELECT COUNT(tag) FROM TagEntry WHERE matchingTagEntry_id IS NULL and owner_id = :ownerId");
	...
	public int countMatchedTagsByUser(int ownerId) {
		Query q = getEntityManager().createQuery("SELECT COUNT(tag) FROM TagEntry WHERE matchingTagEntry_id IS NOT NULL and owner_id = :ownerId");
		q.setParameter("ownerId", ownerId);
		Object result = q.getSingleResult();
		if (result != null) {
			return (int) (long) (Long) q.getSingleResult();
	...
	public int countDictionaryTagsByUser(int ownerId) {
		Query q = getEntityManager().createQuery("SELECT COUNT(tag) FROM TagEntry WHERE dictionary IS NOT NULL and owner_id = :ownerId");
		q.setParameter("ownerId", ownerId);
		Object result = q.getSingleResult();
		if (result != null) {
			return (int) (long) (Long) q.getSingleResult();
	...
	public int countPioneerTagsByUser(int ownerId) {
		Query q = getEntityManager().createQuery("SELECT COUNT(tag) FROM TagEntry WHERE pioneer = 1 and owner_id = :ownerId and matchingTagEntry_id != NULL");
		q.setParameter("ownerId", ownerId);
		Object result = q.getSingleResult();
		if (result != null) {
			return (int) (long) (Long) q.getSingleResult();
	...
	public int countAllTagsWithinMonth(int ownerId, int month) {
		Query q = getEntityManager().createQuery("SELECT COUNT(normalizedTag) FROM TagEntry WHERE MONTH(creationDate) = :month and owner_id = :ownerId GROUP BY owner_id");
		q.setParameter("ownerId", ownerId);
		...
		if (resultList != null && resultList.size() > 0) {
			...
			return (int) (long) (Long) firstResult;
		}
		
		return 0;
	}
public int countAllHoursWithinMonthTotal(int month) {
		Query q = getEntityManager().createQuery("SELECT SUM(tag.game.video.duration) FROM TagEntry WHERE MONTH(creationDate) = :month");
		q.setParameter("month", month);
...
public int countAllTagsWithinMonthTotal(int month) {
		Query q = getEntityManager().createQuery("SELECT COUNT(normalizedTag) FROM TagEntry WHERE MONTH(creationDate) = :month");
		q.setParameter("month", month);
